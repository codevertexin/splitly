...
serve(async (req) => {
  try {
    ...
    if (!expense_id) return jsonResponse({ error: "Missing expense_id" }, 400);
    ...
    if (invalidParticipant) return jsonResponse({ error: "One or more participants are not valid group members" }, 400);
    ...
    if (split_method === "equal") {
      ...
      if (!splits.length) return jsonResponse({ error: "Manual split requires splits[]" }, 400);
      const total = splits.reduce((sum, s) => sum + (s.share_cents || 0), 0);
      if (total !== amount_cents) return jsonResponse({ error: "Manual splits must sum exactly to amount_cents" }, 400);
      splitRows = splits.map((s) => ({ user_id: s.user_id, share_cents: s.share_cents || 0, percentage: null }));
    } else {
      if (!splits.length) return jsonResponse({ error: "Percentage split requires splits[]" }, 400);
      const totalPct = splits.reduce((sum, s) => sum + (s.percentage || 0), 0);
      if (Math.abs(totalPct - 100) > 0.01) return jsonResponse({ error: "Percentages must sum to 100" }, 400);
      let allocated = 0;
    ...
    for (const row of splitRows) {
      if (!validUserIds.has(row.user_id)) return jsonResponse({ error: "Split contains invalid user_id" }, 400);
    }
    ...
    const { error: updateError } = await admin
      ...
      .eq("id", expense_id);
    if (updateError) return jsonResponse({ error: "Failed to update expense", details: updateError.message }, 400);

    const { error: deleteSplitsError } = await admin
      .from("expense_splits")
      .delete()
      .eq("expense_id", expense_id);
    if (deleteSplitsError) return jsonResponse({ error: "Failed to reset expense splits", details: deleteSplitsError.message }, 400);
    ...
    if (insertSplitsError) return jsonResponse({ error: "Failed to save expense splits", details: insertSplitsError.message }, 400);
